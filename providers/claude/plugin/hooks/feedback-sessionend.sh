#!/usr/bin/env bash
# feedback-sessionend.sh — plugin SessionEnd hook for
# feedback-eval/stop-and-subagent-stop.
#
...
# ones that never hit a normal Stop or SubagentStop. Never blocks (SessionEnd
# has no decision control anyway), never calls `stripe feedback`, and does no
